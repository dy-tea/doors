...
static struct wlr_box get_leaf_rect(node_t *node) {
  if (node) {
  	if (node->client) return node->client->tiled_rectangle;
...
static enum wlr_edges determine_drop_edge(node_t *target, double lx, double ly) {
  ...
  if (dist_left < threshold && dist_left <= dist_right &&
      dist_left <= dist_top && dist_left <= dist_bottom)
    return WLR_EDGE_LEFT;
  if (dist_right < threshold && dist_right <= dist_left &&
      dist_right <= dist_top && dist_right <= dist_bottom)
    return WLR_EDGE_RIGHT;
  if (dist_top < threshold && dist_top <= dist_bottom &&
      dist_top <= dist_left && dist_top <= dist_right)
    return WLR_EDGE_TOP;
  if (dist_bottom < threshold && dist_bottom <= dist_top &&
      dist_bottom <= dist_left && dist_bottom <= dist_right)
...
static void update_indicator(enum wlr_edges edge, node_t *target) {
  ...
  case WLR_EDGE_RIGHT:
    iw = 4; ih = rect.height;
    wlr_scene_rect_set_size(server.tiling_drag_indicator, iw, ih);
    wlr_scene_node_set_position(&server.tiling_drag_indicator->node,
      rect.x + rect.width - iw, rect.y);
    break;
  case WLR_EDGE_TOP:
    iw = rect.width; ih = 4;
    wlr_scene_rect_set_size(server.tiling_drag_indicator, iw, ih);
...
void tiling_drag_begin(node_t *node) {
  ...
  if (!server.tiling_drag_indicator) {
    float color[4];
    parse_color(tiling_drag_indicator_color, color);
    server.tiling_drag_indicator = wlr_scene_rect_create(server.drag_tree, 0, 0, color);
  }
  wlr_scene_node_set_enabled(&server.tiling_drag_indicator->node, false);
}

void tiling_drag_motion(void) {
  if (!server.tiling_drag_node) return;
...
void tiling_drag_finish(void) {
  ...
  wlr_log(WLR_DEBUG, "tiling_drag_finish: dragged=%u target=%u edge=%d",
    dragged->id, target->id, server.tiling_drag_target_edge);
  ...
  if (edge == WLR_EDGE_NONE) {
    ...
    direction_t dir;
    switch (edge) {
    case WLR_EDGE_LEFT:   dir = DIR_WEST;  break;
    case WLR_EDGE_RIGHT:  dir = DIR_EAST;  break;
    case WLR_EDGE_TOP:    dir = DIR_NORTH; break;
    case WLR_EDGE_BOTTOM: dir = DIR_SOUTH; break;
    default:              dir = DIR_EAST;  break;
    }
