...
static bool rebuild_live_acrylic(output_t *output, pixman_region32_t *damage, uint64_t shared_capture) {
	...
	// scratch region reused across iterations to avoid per-toplevel malloc/free churn
	pixman_region32_t overlap_rgn;
	pixman_region32_init(&overlap_rgn);
...
static uint64_t capture_bg_cm(output_t *output, effects_output_t *ctx) {
	...
	wlr_damage_ring_add_whole(&ctx->capture_scene_output->damage_ring);
	wlr_output_state_set_enabled(&ctx->capture_state, true);
	wlr_output_state_set_custom_mode(&ctx->capture_state, w, h, 0);
	bool ok = wlr_scene_output_build_state(ctx->capture_scene_output, &ctx->capture_state, NULL);
...
void effects_output_frame(output_t *output, struct wlr_scene_output *scene_output) {
	...
	// scratch regions reused by the layer-blur damage scan below
	pixman_region32_t lm_blur_rgn, lm_intersection;
	pixman_region32_init(&lm_blur_rgn);
	pixman_region32_init(&lm_intersection);
...
after_capture:
	...
	pixman_region32_fini(&lm_blur_rgn);
	pixman_region32_fini(&lm_intersection);
}
