...
static bool ensure_sized_buf(
    struct wlr_buffer **buf_out, uint64_t native[2], int *w_stored, int *h_stored, int w, int h) {
...
void effects_output_resize(effects_output_t *ctx, int width, int height, output_t *output) {
	...
	if (output) {
		for (int i = 0; i < 4; i++) {
			layer_surface_t *ls;
			wl_list_for_each(ls, &output->layers[i], link) effects_destroy_buffer(&ls->blur_buf, ls->blur_native);
		}
	}
...
static bool compute_src_box(
    output_t *output, const struct wlr_box *r, struct wlr_fbox *src_out, int *dw_out, int *dh_out) {
	...
	*src_out = (struct wlr_fbox){.x = sx, .y = sy, .width = sw, .height = sh};
	*dw_out = (int)sw;
	*dh_out = (int)sh;
	return true;
}
...
static uint64_t capture_bg_shared(output_t *output, effects_output_t *ctx) {
	...
	wl_list_for_each(tl, &server.toplevels, link) if (tl->blur && tl->blur->blur_scene_hidden) {
	...
	uint64_t result;
	effects_backend->capture_readback(ctx->capture_state.buffer, &ctx->be_state, ctx->be_state.pong.native_handle[0],
...
static uint64_t capture_bg_combined(output_t *output, effects_output_t *ctx) {
	...
	for (int i = 0; i < 4; i++) {
		layer_surface_t *ls;
		wl_list_for_each(ls, &output->layers[i], link) if (ls->blur_scene_hidden)
...
static bool rebuild_live_blur(output_t *output, pixman_region32_t *damage, uint64_t shared_blurred) {
	...
	wl_list_for_each(tl, &server.toplevels, link) {
		...
		if (effects_state.backend != &vk_backend && c && c->border_radius > 0.0f && c->state != STATE_FULLSCREEN) {
			...
			if (ls->blur_buf && damage && !pixman_region32_empty(damage)) {
				...
				if (nboxes_check > 0) {
					...
					if (wlr_scene_node_coords(&ls->scene_tree->node, &lx_check, &ly_check)) {
						...
						pixman_region32_init_rect(&blur_rgn, boxes_check[0].x1 + out_lx_check, boxes_check[0].y1 + out_ly_check,
						    boxes_check[0].x2 - boxes_check[0].x1, boxes_check[0].y2 - boxes_check[0].y1);
						for (int b = 1; b < nboxes_check; b++)
							pixman_region32_union_rect(&blur_rgn, &blur_rgn, boxes_check[b].x1 + out_lx_check,
							    boxes_check[b].y1 + out_ly_check, boxes_check[b].x2 - boxes_check[b].x1,
...
static bool rebuild_corner_masks(output_t *output, uint64_t bg_tex) {
	...
	wl_list_for_each(tl, &server.toplevels, link) {
		...
		int surf_w = (tl->geometry.width > 0 && tl->geometry.width < container_r.width) ? (int)tl->geometry.width
		                                                                                : container_r.width;
		int surf_h = (tl->geometry.height > 0 && tl->geometry.height < container_r.height) ? (int)tl->geometry.height
		                                                                                   : container_r.height;
...
#undef HIDE_IF
		...
		effects_backend->apply_corner_mask(&ctx->be_state, tl->rounded->corner_mask_native[0], w, h, src, &params);
...
static uint64_t capture_full_scene_to_tex(output_t *output, effects_output_t *ctx) {
	...
	uint64_t result;
	effects_backend->capture_readback(ctx->capture_state.buffer, &ctx->be_state, screen_fbo, w, h, w, h, &result);
...
static void handle_screen_shader_frame(output_t *output) {
	...
	struct be_screen_shader_params ssp = {.type = SCREEN_SHADER_CUSTOM,
...
void effects_output_frame(output_t *output, struct wlr_scene_output *scene_output) {
		...
		wl_list_for_each(tl, &server.toplevels, link) {
			...
			if (tl->shadow->shadow_geometry_dirty && !tl->shadow->shadow_dirty) {
				...
				if (tl->shadow->shadow_node && size > 0) {
					...
					if (buf_w > 0 && buf_h > 0) {
						float scale = tl->node->output ? tl->node->output->wlr_output->scale : 1.0f;
						if (tl->shadow->shadow_buf_w == (int)(buf_w * scale) && tl->shadow->shadow_buf_h == (int)(buf_h * scale)) {
							wlr_scene_node_lower_to_bottom(&tl->shadow->shadow_node->node);
	...
	bool any_cm_dirty = false;
	{
		...
		wl_list_for_each(tl, &server.toplevels, link) {
			if (tl->rounded && tl->rounded->corner_mask_node && tl->node && tl->node->client
			    && tl->node->client->border_radius > 0.0f && tl->node->client->state != STATE_FULLSCREEN && tl->node->output
			    && tl->node->output == output) {
				any_cm = true;
	...
	uint64_t shared_capture = 0;
	uint64_t shared_blurred = 0;
	{
		bool any_toplevel_effect = false;
		if (bg_damaged) {
			toplevel_t *tl;
			wl_list_for_each(tl, &server.toplevels, link) {
				if (!tl->blur || !tl->node || !tl->node->client || !tl->node->client->shown)
					continue;
				if (tl->node->output != output)
					continue;
		...
		if (any_toplevel_effect) {
			...
			if (shared_capture && blur_enabled) {
				...
				effects_backend->blur(&ctx->be_state, shared_capture, ctx->blur_w, ctx->blur_h, &bp, &shared_blurred);
			}
		}
	}
	...
	if (blur_enabled) {
		...
		toplevel_t *tl;
		wl_list_for_each(tl, &server.toplevels, link) {
			if (tl->blur && tl->blur->blur_node && tl->node && tl->node->client && tl->node->client->shown && tl->node->output
			    && tl->node->output == output) {
				any_blur = true;
				break;
			}
		...
		if (blur_enabled && bg_damaged) {
			...
			for (int i = 0; i < 4 && !any_layer_needs_blur; i++) {
				...
				wl_list_for_each(ls, &output->layers[i], link) {
					...
					pixman_region32_t blur_rgn, intersection;
					pixman_region32_init_rect(&blur_rgn, boxes[0].x1 + out_lx, boxes[0].y1 + out_ly, boxes[0].x2 - boxes[0].x1,
					    boxes[0].y2 - boxes[0].y1);
					for (int b = 1; b < nboxes; b++)
						pixman_region32_union_rect(&blur_rgn, &blur_rgn, boxes[b].x1 + out_lx, boxes[b].y1 + out_ly,
...
const char *screen_shader_get_name(void) { return screen_shader_name_str; }
