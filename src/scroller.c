	...
	for (int i = 0; i < n; i++) {
		...
		if (c->scroller_proportion <= 0.0f && c->toplevel && c->toplevel->xdg_toplevel) {
			...
			if (geom.width > 0 && geom.height > 0) {
				...
				if (requested_proportion < 0.1f)
					requested_proportion = 0.1f;
				if (requested_proportion > 1.0f)
...
void scroller_resize_width(client_t *client, float delta) {
	...
	if (new_proportion < 0.1f)
		new_proportion = 0.1f;
	if (new_proportion > 1.0f)
		new_proportion = 1.0f;
...
void scroller_apply_client_rules(client_t *c, float rule_proportion, float rule_proportion_single) {
	...
	if (rule_proportion > 0.0f) {
		if (rule_proportion < 0.1f)
			rule_proportion = 0.1f;
		if (rule_proportion > 1.0f)
			rule_proportion = 1.0f;
	...
	if (rule_proportion_single > 0.0f) {
		if (rule_proportion_single < 0.1f)
			rule_proportion_single = 0.1f;
		if (rule_proportion_single > 1.0f)
...
bool scroller_focus_next(desktop_t *d) {
	if (!d || !d->root || !d->focus || !d->focus->client)
		return false;
...
bool scroller_focus_down(desktop_t *d) {
	if (!d || !d->focus || !d->focus->client)
		return false;

	client_t *current = d->focus->client;
	if (!current->next_in_stack)
		return false;
