...
union vk_push_data {
  struct { float d[32]; } raw;
  struct { float hp[2]; float off; float noise; float vib; float vd; float br; float cont; } kawase;
  struct { float texel[2]; float radius; float _pad0; float vib; float vd; float br; float cont; } gauss;
  struct { float tint[4]; float strength; } mica;
  struct { float tint[4]; float strength; float noise; float res[2]; float anchor[2]; } acrylic;
  struct { float hp[2]; float off; float rsize[2]; float edge; float corner; float str; float npow; float rf; int rm; int mode; int _pad0; } refraction;
  struct { float res[2]; float size; float _pad; float col[4]; float br; float _pad2; float inner[2]; float hole[2]; float hsize[2]; } shadow;
  struct { float wpos[2]; float wsz[2]; float wpx[2]; float br; float scale; int pre; } corner;
  struct { float res[2]; float time; float _pad[2]; } screen;
};
...
static VkShaderModule vk_compile_shader(const char *glsl_src, VkShaderStageFlagBits stage) {
  ...
  if (!compiler) { wlr_log(WLR_ERROR, "vk: failed to init shaderc"); return VK_NULL_HANDLE; }

  shaderc_shader_kind kind = (stage == VK_SHADER_STAGE_VERTEX_BIT) ? shaderc_glsl_vertex_shader : shaderc_glsl_fragment_shader;
  ...
  if (vkCreateShaderModule(vk->device, &ci, NULL, &module) != VK_SUCCESS) module = VK_NULL_HANDLE;
  shaderc_result_release(result);
  shaderc_compiler_release(compiler);
  return module;
}
...
static VkPipeline vk_create_pipe(VkShaderModule frag_mod, bool use_border_layout, bool blend_enable) {
  VkPipelineShaderStageCreateInfo stages[2] = {
    {.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO,
      .stage = VK_SHADER_STAGE_VERTEX_BIT, .module = vk->vert_module, .pName = "main"},
    {.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO,
...
static bool vk_create_image(int w, int h, VkFormat fmt, VkImageUsageFlags usage, struct vk_image *out) {
  VkImageCreateInfo ci = {
    ...
    .extent = {w, h, 1},
    .mipLevels = 1, .arrayLayers = 1, .format = fmt,
    .tiling = VK_IMAGE_TILING_OPTIMAL, .initialLayout = VK_IMAGE_LAYOUT_UNDEFINED,
    .usage = usage, .samples = VK_SAMPLE_COUNT_1_BIT,
  };
  ...
  VkImageViewCreateInfo vci = {
    ...
    .image = out->image, .viewType = VK_IMAGE_VIEW_TYPE_2D, .format = fmt,
...
static void vk_transition_to_shader_read(VkImage image) {
  ...
  VkCommandBuffer cb;
  if (vkAllocateCommandBuffers(vk->device, &cai, &cb) != VK_SUCCESS) return;
...
static void vk_destroy_image(struct vk_image *img) {
  if (img->view) { vkDestroyImageView(vk->device, img->view, NULL); img->view = VK_NULL_HANDLE; }
  if (img->image) { vkDestroyImage(vk->device, img->image, NULL); img->image = VK_NULL_HANDLE; }
  if (img->memory) { vkFreeMemory(vk->device, img->memory, NULL); img->memory = VK_NULL_HANDLE; }
}

static bool vk_create_fbo(int w, int h, VkFormat fmt, struct vk_fbo *out) {
  if (!vk_create_image(w, h, fmt,
      VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT |
      VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT, &out->img))
    return false;
  vk_transition_to_shader_read(out->img.image);
  VkFramebufferCreateInfo fci = {
    .sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO,
    .renderPass = vk->render_pass, .attachmentCount = 1,
    .pAttachments = &out->img.view, .width = w, .height = h, .layers = 1,
  };
...
static void vk_destroy_fbo(struct vk_fbo *fbo) {
  if (fbo->fb) { vkDestroyFramebuffer(vk->device, fbo->fb, NULL); fbo->fb = VK_NULL_HANDLE; }
  vk_destroy_image(&fbo->img);
}

static void vk_draw_full(VkPipeline pipe, VkImage src_img, VkFramebuffer dst_fb, int w, int h,
    VkImage dst_img, const void *pc_data, size_t pc_size, const VkRect2D *scissor, uint32_t n_scissor) {
  VkCommandBufferBeginInfo bi = {
    .sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO,
    .flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT,
  };
  if (vkBeginCommandBuffer(vk->frame_cb, &bi) != VK_SUCCESS) {
    wlr_log(WLR_ERROR, "vk: draw_full begin failed");
    return;
  }
  ...
  VkRenderPassBeginInfo rp = {
    ...
    .renderPass = vk->render_pass, .framebuffer = dst_fb,
    .renderArea = {{0, 0}, {w, h}},
...
static void vk_draw_full_no_tex(VkPipeline pipe, VkImage dst_img, VkFramebuffer dst_fb,
  ...
  if (vkBeginCommandBuffer(vk->frame_cb, &bi) != VK_SUCCESS) {
    wlr_log(WLR_ERROR, "vk: draw_full_no_tex begin failed");
    return;
  }
  ...
  vkCmdSetViewport(vk->frame_cb, 0, 1, &vp);
  vkCmdSetScissor(vk->frame_cb, 0, 1, &sc);
  if (ds) vkCmdBindDescriptorSets(vk->frame_cb, VK_PIPELINE_BIND_POINT_GRAPHICS,
...
static bool vk_init(struct wlr_renderer *r, struct wlr_allocator *a) {
  ...
  VkCommandBufferAllocateInfo cai = {
    .sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO,
    .commandPool = vk->cmd_pool, .level = VK_COMMAND_BUFFER_LEVEL_PRIMARY, .commandBufferCount = 1,
  };
  if (vkAllocateCommandBuffers(vk->device, &cai, &vk->frame_cb) != VK_SUCCESS)
    { vkDestroyCommandPool(vk->device, vk->cmd_pool, NULL); free(vk); vk = NULL; return false; }

  VkAttachmentDescription att = {
    .format = vk->vk_fmt, .samples = VK_SAMPLE_COUNT_1_BIT,
  ...
  if (vkCreateDescriptorSetLayout(vk->device, &dsci, NULL, &vk->ds_layout) != VK_SUCCESS) return false;
  ...
  VkDescriptorSetLayoutCreateInfo bdsci = {
    .sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO,
    .bindingCount = 2, .pBindings = bbind,
  };
  if (vkCreateDescriptorSetLayout(vk->device, &bdsci, NULL, &vk->ds_border_layout) != VK_SUCCESS) return false;

  VkPushConstantRange pcr = {VK_SHADER_STAGE_FRAGMENT_BIT, 0, 128};
  VkPipelineLayoutCreateInfo plci = {
    .sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO,
    .setLayoutCount = 1, .pSetLayouts = &vk->ds_layout,
    .pushConstantRangeCount = 1, .pPushConstantRanges = &pcr,
  };
  if (vkCreatePipelineLayout(vk->device, &plci, NULL, &vk->pipe_layout) != VK_SUCCESS) return false;

  plci.pSetLayouts = &vk->ds_border_layout;
  if (vkCreatePipelineLayout(vk->device, &plci, NULL, &vk->border_pipe_layout) != VK_SUCCESS) return false;
  ...
  if (!vk->vert_module || !vk->frag_blit || !vk->frag_kawase || !vk->frag_corner_mask)
    { wlr_log(WLR_ERROR, "vk: shaders failed to compile"); return false; }
  ...
  if (!vk->pipe_blit || !vk->pipe_kawase || !vk->pipe_corner_mask)
    { wlr_log(WLR_ERROR, "vk: pipelines failed to create"); return false; }

  // border UBO
  VkBufferCreateInfo bci = {
    .sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO,
    .size = sizeof(struct vk_border_ubo), .usage = VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
  };
  if (vkCreateBuffer(vk->device, &bci, NULL, &vk->border_ubo) != VK_SUCCESS) return false;
  ...
  if (vkAllocateMemory(vk->device, &bai, NULL, &vk->border_ubo_mem) != VK_SUCCESS)
    { vkDestroyBuffer(vk->device, vk->border_ubo, NULL); return false; }
  vkBindBufferMemory(vk->device, vk->border_ubo, vk->border_ubo_mem, 0);

  VkDescriptorPoolSize bp_sz[] = {{VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1}, {VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1}};
  VkDescriptorPoolCreateInfo bpci = {
    .sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO,
    .maxSets = 1, .poolSizeCount = 2, .pPoolSizes = bp_sz,
  };
  if (vkCreateDescriptorPool(vk->device, &bpci, NULL, &vk->border_ds_pool) != VK_SUCCESS) return false;
  VkDescriptorSetAllocateInfo bdsai = {
    .sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO,
    .descriptorPool = vk->border_ds_pool, .descriptorSetCount = 1,
    .pSetLayouts = &vk->ds_border_layout,
  };
  if (vkAllocateDescriptorSets(vk->device, &bdsai, &vk->border_ds) != VK_SUCCESS) return false;
  VkDescriptorBufferInfo bdbi = {.buffer = vk->border_ubo, .offset = 0, .range = sizeof(struct vk_border_ubo)};
  VkWriteDescriptorSet bw = {
    .sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,
    .dstSet = vk->border_ds, .dstBinding = 1, .descriptorCount = 1,
    .descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, .pBufferInfo = &bdbi,
  };
  ...
  if (vkCreateDescriptorPool(vk->device, &dpci, NULL, &vk->desc_pool) != VK_SUCCESS) return false;
  ...
  if (vkCreateImage(vk->device, &dici, NULL, &vk->dummy_img) != VK_SUCCESS) return false;
  ...
  VkMemoryAllocateInfo dai = {
    ...
    .memoryTypeIndex = vk_find_mem_type(vk->phys_dev,
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, dmr.memoryTypeBits),
  };
  if (vkAllocateMemory(vk->device, &dai, NULL, &vk->dummy_img_mem) != VK_SUCCESS) return false;
  ...
  if (vkCreateBuffer(vk->device, &sbci, NULL, &staging) != VK_SUCCESS) return false;
  ...
  if (vkAllocateMemory(vk->device, &smai, NULL, &staging_mem) != VK_SUCCESS)
    { vkDestroyBuffer(vk->device, staging, NULL); return false; }
  vkBindBufferMemory(vk->device, staging, staging_mem, 0);
  void *map; vkMapMemory(vk->device, staging_mem, 0, 4, 0, &map);
  memcpy(map, &white, 4); vkUnmapMemory(vk->device, staging_mem);
  ...
  vkCmdPipelineBarrier(tcb, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, NULL, 0, NULL, 1, &imb2);
  ...
  VkImageViewCreateInfo dvi = {
    .sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO,
    .image = vk->dummy_img, .viewType = VK_IMAGE_VIEW_TYPE_2D,
    .format = vk->vk_fmt,
    .subresourceRange = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1},
  };
  if (vkCreateImageView(vk->device, &dvi, NULL, &vk->dummy_view) != VK_SUCCESS) return false;

  // write dummy sampler to border descriptor set binding 0
  VkDescriptorImageInfo dummy_dii = {
    .sampler = vk->sampler, .imageView = vk->dummy_view,
    .imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
  };
  VkWriteDescriptorSet write_dummy = {
    .sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,
    .dstSet = vk->border_ds, .dstBinding = 0, .descriptorCount = 1,
    .descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, .pImageInfo = &dummy_dii,
  };
  ...
  VkDescriptorPoolCreateInfo ddpci = {.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO, .maxSets = 1, .poolSizeCount = 1, .pPoolSizes = &ddps};
  if (vkCreateDescriptorPool(vk->device, &ddpci, NULL, &vk->dummy_ds_pool) != VK_SUCCESS) return false;
  VkDescriptorSetAllocateInfo ddsai = {
    .sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO,
    .descriptorPool = vk->dummy_ds_pool, .descriptorSetCount = 1,
    .pSetLayouts = &vk->ds_layout,
  };
  if (vkAllocateDescriptorSets(vk->device, &ddsai, &vk->dummy_ds) != VK_SUCCESS) return false;
  VkWriteDescriptorSet write_dummy2 = {
    .sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,
    .dstSet = vk->dummy_ds, .dstBinding = 0, .descriptorCount = 1,
    .descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, .pImageInfo = &dummy_dii,
  };
...
#define DESTROY_PIPE(p) do { if (p) { vkDestroyPipeline(vk->device, p, NULL); p = VK_NULL_HANDLE; } } while(0)
#define DESTROY_MOD(m) do { if (m) { vkDestroyShaderModule(vk->device, m, NULL); m = VK_NULL_HANDLE; } } while(0)
  if (vk->screen_shader_pipe) { vkDestroyPipeline(vk->device, vk->screen_shader_pipe, NULL); }
  if (vk->screen_shader_module) { vkDestroyShaderModule(vk->device, vk->screen_shader_module, NULL); }
  DESTROY_PIPE(vk->pipe_blit); DESTROY_PIPE(vk->pipe_kawase);
  DESTROY_PIPE(vk->pipe_gauss_h); DESTROY_PIPE(vk->pipe_gauss_v);
  DESTROY_PIPE(vk->pipe_box_h); DESTROY_PIPE(vk->pipe_box_v);
  DESTROY_PIPE(vk->pipe_mica); DESTROY_PIPE(vk->pipe_acrylic);
  DESTROY_PIPE(vk->pipe_refraction); DESTROY_PIPE(vk->pipe_shadow);
  DESTROY_PIPE(vk->pipe_border); DESTROY_PIPE(vk->pipe_corner_mask);
  DESTROY_MOD(vk->vert_module); DESTROY_MOD(vk->frag_blit); DESTROY_MOD(vk->frag_kawase);
  DESTROY_MOD(vk->frag_gauss_h); DESTROY_MOD(vk->frag_gauss_v);
  DESTROY_MOD(vk->frag_box_h); DESTROY_MOD(vk->frag_box_v);
  DESTROY_MOD(vk->frag_mica); DESTROY_MOD(vk->frag_acrylic);
  DESTROY_MOD(vk->frag_refraction); DESTROY_MOD(vk->frag_shadow);
  DESTROY_MOD(vk->frag_border); DESTROY_MOD(vk->frag_corner_mask);
#undef DESTROY_PIPE
#undef DESTROY_MOD

  if (vk->dummy_view) vkDestroyImageView(vk->device, vk->dummy_view, NULL);
  if (vk->dummy_img_mem) vkFreeMemory(vk->device, vk->dummy_img_mem, NULL);
  if (vk->dummy_img) vkDestroyImage(vk->device, vk->dummy_img, NULL);
  if (vk->dummy_ds_pool) vkDestroyDescriptorPool(vk->device, vk->dummy_ds_pool, NULL);
  if (vk->border_ds_pool) vkDestroyDescriptorPool(vk->device, vk->border_ds_pool, NULL);
  if (vk->border_ubo_mem) vkFreeMemory(vk->device, vk->border_ubo_mem, NULL);
  if (vk->border_ubo) vkDestroyBuffer(vk->device, vk->border_ubo, NULL);
  if (vk->desc_pool) vkDestroyDescriptorPool(vk->device, vk->desc_pool, NULL);
  if (vk->sampler) vkDestroySampler(vk->device, vk->sampler, NULL);
  if (vk->border_pipe_layout) vkDestroyPipelineLayout(vk->device, vk->border_pipe_layout, NULL);
  if (vk->pipe_layout) vkDestroyPipelineLayout(vk->device, vk->pipe_layout, NULL);
  if (vk->ds_border_layout) vkDestroyDescriptorSetLayout(vk->device, vk->ds_border_layout, NULL);
  if (vk->ds_layout) vkDestroyDescriptorSetLayout(vk->device, vk->ds_layout, NULL);
  if (vk->render_pass) vkDestroyRenderPass(vk->device, vk->render_pass, NULL);
  if (vk->color_clear_render_pass) vkDestroyRenderPass(vk->device, vk->color_clear_render_pass, NULL);
  if (vk->overlay_render_pass) vkDestroyRenderPass(vk->device, vk->overlay_render_pass, NULL);
...
static bool vk_output_init(be_output_state_t *state, int width, int height,
  ...
  state->staging.width = width; state->staging.height = height;
  ...
  if (ss && vk_create_fbo(width, height, vk->vk_fmt, ss)) {
    ...
    state->screen_shader.width = width; state->screen_shader.height = height;
  } else {
    free(ss);
...
static bool vk_ensure_buffer(struct wlr_buffer **buf, uint64_t native[2],
  ...
  if (*buf) return native[0] != 0;
  if (!vk->render_fmt) return false;
  ...
  fbo->img.image = vk_attribs.image; fbo->img.memory = VK_NULL_HANDLE;
  fbo->img.width = w; fbo->img.height = h;
  fbo->tex = tex;

  vk_transition_to_shader_read(vk_attribs.image);

  VkFormat actual_fmt = vk_attribs.format;
  if (actual_fmt == VK_FORMAT_UNDEFINED) actual_fmt = vk->vk_fmt;
  if (actual_fmt != vk->vk_fmt)
    wlr_log(WLR_INFO, "vk: ensure_buffer format mismatch: render_pass=%d actual=%d",
      vk->vk_fmt, actual_fmt);

  VkImageViewCreateInfo vci = {
    .sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO,
    .image = vk_attribs.image, .viewType = VK_IMAGE_VIEW_TYPE_2D,
    .format = actual_fmt,
    .subresourceRange = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1},
  };
  ...
  VkFramebufferCreateInfo fci = {
    .sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO,
    .renderPass = vk->render_pass, .attachmentCount = 1, .pAttachments = &fbo->img.view,
...
static void vk_destroy_buffer(struct wlr_buffer *buf, uint64_t native[2]) {
  ...
  if (fbo) {
    if (fbo->img.view) vkDestroyImageView(vk->device, fbo->img.view, NULL);
    if (fbo->fb) vkDestroyFramebuffer(vk->device, fbo->fb, NULL);
    if (fbo->tex) wlr_texture_destroy(fbo->tex);
    free(fbo);
  }
...
static bool vk_blit(uint64_t src_tex, uint64_t dst_fbo, int w, int h,
  ...
  VkCommandBuffer cb;
  if (vkAllocateCommandBuffers(vk->device, &cai, &cb) != VK_SUCCESS) return false;
  VkCommandBufferBeginInfo bi = {.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO, .flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT};
  vkBeginCommandBuffer(cb, &bi);
  ...
  vkCmdPipelineBarrier(cb, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, NULL, 0, NULL, 1, &src_barrier);
  ...
  vkCmdPipelineBarrier(cb, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, NULL, 0, NULL, 1, &dst_barrier);
  ...
  for (int i = 0; i < n_regions; i++) {
    ...
    vkCmdBlitImage(cb, src_img, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
      dst->img.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region, VK_FILTER_LINEAR);
  }

  VkImageMemoryBarrier src_barrier2 = {
    .sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER,
  ...
  vkCmdPipelineBarrier(cb, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, NULL, 0, NULL, 1, &src_barrier2);
  ...
  vkCmdPipelineBarrier(cb, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, NULL, 0, NULL, 1, &dst_barrier2);
...
static bool vk_blur(be_output_state_t *state, uint64_t src_handle,
  int src_w, int src_h, struct be_blur_params *p, uint64_t *out_handle) {
  wlr_log(WLR_INFO, "vk: blur src=%p w=%d h=%d passes=%d algo=%d",
    (void*)src_handle, src_w, src_h, p->passes, p->algorithm);
  ...
  for (int i = 0; i < p->passes; i++) {
    if (p->algorithm == BLUR_ALGORITHM_GAUSSIAN) {
      ...
      if (fringing < 0) fringing = 0;
      if (fringing > 1) fringing = 1;
...
static bool vk_apply_acrylic(be_output_state_t *state,
  ...
  if (p->blur_passes > 0) {
    ...
    for (int i = 0; i < p->blur_passes; i++) {
      struct { float hp[2]; float off; float _pad[28]; } pc = {0};
      pc.hp[0] = 0.5f / blur_w; pc.hp[1] = 0.5f / blur_h;
  ...
  struct { float tint[4]; float strength; float noise; float res[2]; float anchor[2]; } pc;
...
static bool vk_render_shadow(struct be_shadow_params *p, uint64_t dst_fbo) {
  struct vk_fbo *dst = vk_fbo_of(dst_fbo);
  if (!dst) return false;
  struct { float res[2]; float size; float _pad; float col[4]; float br; float _pad2; float inner[2]; float hole[2]; float hsize[2]; } pc;
  memset(&pc, 0, sizeof(pc));
  pc.res[0] = (float)p->buf_w; pc.res[1] = (float)p->buf_h;
  pc.size = p->shadow_size; memcpy(pc.col, p->shadow_color, 16);
  pc.br = p->border_radius; pc.inner[0] = p->inner_width; pc.inner[1] = p->inner_height;
  pc.hole[0] = p->hole_x; pc.hole[1] = (float)p->buf_h - p->hole_y - p->hole_height;
  pc.hsize[0] = p->hole_width; pc.hsize[1] = p->hole_height;

  struct vk_fbo tmp;
  if (!vk_create_fbo(p->buf_w, p->buf_h, vk->vk_fmt, &tmp)) return false;
  vk_draw_full_no_tex(vk->pipe_shadow, tmp.img.image, tmp.fb, p->buf_w, p->buf_h, true, &pc, sizeof(pc), vk->pipe_layout, vk->dummy_ds);
  ...
  VkImageMemoryBarrier src_b = {.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER, .oldLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, .newLayout = VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, .srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED, .dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED, .image = tmp.img.image, .subresourceRange = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1}, .srcAccessMask = VK_ACCESS_SHADER_READ_BIT, .dstAccessMask = VK_ACCESS_TRANSFER_READ_BIT};
  vkCmdPipelineBarrier(cb, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, NULL, 0, NULL, 1, &src_b);

  VkImageMemoryBarrier dst_to = {.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER, .oldLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, .newLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, .srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED, .dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED, .image = dst->img.image, .subresourceRange = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1}, .srcAccessMask = VK_ACCESS_SHADER_READ_BIT, .dstAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT};
  vkCmdPipelineBarrier(cb, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, NULL, 0, NULL, 1, &dst_to);

  VkImageBlit blit = {.srcSubresource = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1}, .srcOffsets = {{0, 0, 0}, {p->buf_w, p->buf_h, 1}}, .dstSubresource = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1}, .dstOffsets = {{0, 0, 0}, {p->buf_w, p->buf_h, 1}}};
  ...
  VkImageMemoryBarrier dst_back = {.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER, .oldLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, .newLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, .srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED, .dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED, .image = dst->img.image, .subresourceRange = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1}, .srcAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT, .dstAccessMask = VK_ACCESS_SHADER_READ_BIT};
  vkCmdPipelineBarrier(cb, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, NULL, 0, NULL, 1, &dst_back);
...
static bool vk_render_border(struct be_border_params *p, uint64_t dst_fbo) {
  ...
  ubo.gradient_count = p->gradient_count; ubo.gradient2_count = p->gradient2_count;
  ubo.gradient_angle = p->gradient_angle; ubo.gradient2_angle = p->gradient2_angle;
  ubo.gradient_lerp = p->gradient_lerp;
  void *mapped;
  vkMapMemory(vk->device, vk->border_ubo_mem, 0, sizeof(ubo), 0, &mapped);
  memcpy(mapped, &ubo, sizeof(ubo));
  vkUnmapMemory(vk->device, vk->border_ubo_mem);

  struct { float res[2]; float br; float bw; float scale; float _pad[3]; float color[4]; } pc;
  memset(&pc, 0, sizeof(pc));
  pc.res[0] = p->res_w; pc.res[1] = p->res_h; pc.br = p->border_radius;
  pc.bw = p->border_width_px; pc.scale = p->scale; memcpy(pc.color, p->border_color, 16);

  struct vk_fbo tmp;
  if (!vk_create_fbo(p->buf_w, p->buf_h, vk->vk_fmt, &tmp)) return false;
  vk_draw_full_no_tex(vk->pipe_border, tmp.img.image, tmp.fb, p->buf_w, p->buf_h, true, &pc, sizeof(pc), vk->border_pipe_layout, vk->border_ds);
  ...
  VkImageMemoryBarrier src_to = {.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER, .oldLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, .newLayout = VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, .srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED, .dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED, .image = tmp.img.image, .subresourceRange = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1}, .srcAccessMask = VK_ACCESS_SHADER_READ_BIT, .dstAccessMask = VK_ACCESS_TRANSFER_READ_BIT};
  vkCmdPipelineBarrier(cb, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, NULL, 0, NULL, 1, &src_to);
...
static bool vk_apply_corner_mask(be_output_state_t *state, uint64_t dst_fbo,
  ...
  struct vk_fbo tmp;
  if (!vk_create_fbo(dst_w, dst_h, vk->vk_fmt, &tmp)) return false;
  VkImage src_img = vk_img_of(bg_tex);

  // allocate descriptors before the command buffer
  VkImageViewCreateInfo vci = {
    .sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO,
    .image = src_img, .viewType = VK_IMAGE_VIEW_TYPE_2D, .format = vk->vk_fmt,
    .subresourceRange = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1},
  ...
  if (vkCreateImageView(vk->device, &vci, NULL, &src_view) != VK_SUCCESS) { vk_destroy_fbo(&tmp); return false; }
  VkDescriptorSetAllocateInfo dsai = {
    .sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO,
    .descriptorPool = vk->desc_pool, .descriptorSetCount = 1,
    .pSetLayouts = &vk->ds_layout,
  ...
  vkAllocateDescriptorSets(vk->device, &dsai, &ds);
  VkDescriptorImageInfo dii = {
    .sampler = vk->sampler, .imageView = src_view,
    .imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
  };
  VkWriteDescriptorSet write = {
    .sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,
  ...
  struct { float wpos[2]; float wsz[2]; float wpx[2]; float br; float scale; int pre; } pc;
  memset(&pc, 0, sizeof(pc));
  pc.wpos[0] = p->win_u; pc.wpos[1] = 1.0f - p->win_v - p->win_sh;
  pc.wsz[0] = p->win_sw; pc.wsz[1] = p->win_sh;
  pc.wpx[0] = p->win_size_px_w; pc.wpx[1] = p->win_size_px_h;
  pc.br = p->border_radius_px; pc.scale = p->scale;
  ...
  VkCommandBuffer cb;
  if (vkAllocateCommandBuffers(vk->device, &cai, &cb) != VK_SUCCESS) { vkDestroyImageView(vk->device, src_view, NULL); vk_destroy_fbo(&tmp); return false; }
  VkCommandBufferBeginInfo bi = {.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO, .flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT};
  vkBeginCommandBuffer(cb, &bi);
  ...
  // blit dst -> tmp: OUT buffer -> temp FBO
  {
    VkImageMemoryBarrier b[2] = {
      {.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER, .oldLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, .newLayout = VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, .image = dst->img.image, .subresourceRange = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1}, .srcAccessMask = VK_ACCESS_SHADER_READ_BIT, .dstAccessMask = VK_ACCESS_TRANSFER_READ_BIT},
      {.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER, .oldLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, .newLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, .image = tmp.img.image, .subresourceRange = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1}, .srcAccessMask = VK_ACCESS_SHADER_READ_BIT, .dstAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT},
    };
  ...
  {
    VkImageMemoryBarrier b[2] = {
      {.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER, .oldLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, .newLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, .image = tmp.img.image, .subresourceRange = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1}, .srcAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT, .dstAccessMask = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT | VK_ACCESS_COLOR_ATTACHMENT_READ_BIT},
      {.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER, .oldLayout = VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, .newLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, .image = dst->img.image, .subresourceRange = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1}, .srcAccessMask = VK_ACCESS_TRANSFER_READ_BIT, .dstAccessMask = VK_ACCESS_SHADER_READ_BIT},
    };
    vkCmdPipelineBarrier(cb, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT | VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, NULL, 0, NULL, 2, b);
  }

  // render pass: overlay corner mask on tmp
  {
    VkRenderPassBeginInfo rp = {
      .sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO,
      .renderPass = vk->overlay_render_pass, .framebuffer = tmp.fb,
    ...
    vkCmdBindDescriptorSets(cb, VK_PIPELINE_BIND_POINT_GRAPHICS, vk->pipe_layout, 0, 1, &ds, 0, NULL);
    vkCmdPushConstants(cb, vk->pipe_layout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(pc), &pc);
    vkCmdBindPipeline(cb, VK_PIPELINE_BIND_POINT_GRAPHICS, vk->pipe_corner_mask);
    vkCmdDraw(cb, 4, 1, 0, 0);
    vkCmdEndRenderPass(cb);
  }

  // blit tmp -> dst: copy corner-masked result back to output buffer
  {
    VkImageMemoryBarrier b[2] = {
  ...
  // transition both back to SHADER_READ_ONLY
  {
    ...
    vkCmdPipelineBarrier(cb, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, NULL, 0, NULL, 2, b);
  }
  ...
  vkDestroyImageView(vk->device, src_view, NULL);
  vk_destroy_fbo(&tmp);
  return true;
}
...
static bool vk_capture_readback(struct wlr_buffer *capture_buffer,
    ...
    int src_w, int src_h, uint64_t *out_tex) {
  ...
  wlr_log(WLR_INFO, "vk: capture layout=%d fmt=%d w=%d h=%d",
    src_layout, src_fmt, dst_w, dst_h);
  ...
  vkCmdPipelineBarrier(cb, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, NULL, 0, NULL, 1, &src_barrier);
  ...
  vkCmdBlitImage(cb, vk_attribs.image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
    dst->img.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region, VK_FILTER_LINEAR);
...
static bool vk_compile_screen_shader(const char *frag_src) {
  ...
  static const struct { const char *from; const char *to; } subs[] = {
    {"precision mediump float;", ""}, {"precision highp float;", ""}, {"precision lowp float;", ""},
    {"varying vec2 v_uv;", "layout(location = 0) in vec2 v_uv;"},
    {"varying vec4 v_uv;", "layout(location = 0) in vec4 v_uv;"},
    {"attribute vec2 pos;", ""}, {"attribute vec4 pos;", ""},
    {"uniform sampler2D tex;", "layout(binding = 0) uniform sampler2D tex;"},
    {"texture2D(", "texture("}, {"gl_FragColor", "fragColor"},
    {"#extension", "//"}, {"#include", "//"},
  };
  ...
  while (*src) {
    ...
    for (size_t i = 0; i < sizeof(subs)/sizeof(subs[0]); i++) {
      ...
      if (strncmp(src, subs[i].from, sl) == 0) {
        ...
        memcpy(dst, subs[i].to, tl); dst += tl; src += sl;
        matched = true; break;
      }
    }
    if (!matched) *dst++ = *src++;
