...
static size_t collect_leaves(node_t *n, node_t **out, size_t cap) {
	...
	size_t used = 0;
	used += collect_leaves(n->first_child, out ? out + used : NULL, cap > used ? cap - used : 0);
	used += collect_leaves(n->second_child, out ? out + used : NULL, cap > used ? cap - used : 0);
	return used;
}
...
void tabs_arrange(node_t *n, struct wlr_box rect) {
	...
	for (int i = 0; i < n_entries; i++) {
		...
		if (e->border) {
			wlr_scene_node_set_position(&e->border->node, 0, TAB_BAR_HEIGHT - TAB_BAR_BORDER);
