...
void tablet_configure(tablet_t *tablet) {
  ...
  wl_list_for_each(pad, &tablet->seat->tablet_pads, link) {
    ...
    if (!wlr_input_device_is_libinput(pad_device)) continue;
...
static void handle_tablet_tool_set_cursor(struct wl_listener *listener, void *data) {
  ...
  if (!focused_client || event->seat_client->client != focused_client) return;
...
tablet_tool_t *tablet_tool_configure(tablet_t *tablet, struct wlr_tablet_tool *wlr_tool) {
  ...
  wlr_log(WLR_DEBUG, "Configured tablet tool type=%d for seat %s",
    wlr_tool->type, tablet->seat->name);
...
static void handle_pad_ring(struct wl_listener *listener, void *data) {
  ...
  if (!pad->current_surface || !pad->tablet_v2_pad) return;

  wlr_tablet_v2_tablet_pad_notify_ring(pad->tablet_v2_pad, event->ring,
    event->position, event->source == WLR_TABLET_PAD_RING_SOURCE_FINGER,
    event->time_msec);
}
...
static void handle_pad_strip(struct wl_listener *listener, void *data) {
  ...
  wlr_tablet_v2_tablet_pad_notify_strip(pad->tablet_v2_pad, event->strip,
  	event->position, event->source == WLR_TABLET_PAD_STRIP_SOURCE_FINGER,
    event->time_msec);
}
...
static void handle_pad_button(struct wl_listener *listener, void *data) {
  ...
  wlr_tablet_v2_tablet_pad_notify_mode(pad->tablet_v2_pad, event->group,
  	event->mode, event->time_msec);

  wlr_tablet_v2_tablet_pad_notify_button(pad->tablet_v2_pad, event->button,
  	event->time_msec, (enum zwp_tablet_pad_v2_button_state)event->state);
}
...
void tablet_pad_configure(tablet_pad_t *pad) {
  ...
  wl_list_for_each(tablet, &pad->seat->tablets, link) {
    ...
    if (!wlr_input_device_is_libinput(tablet_device)) continue;
...
void tablet_pad_set_focus(tablet_pad_t *pad, struct wlr_surface *surface) {
  if (!pad || !pad->tablet_v2_pad || !pad->tablet) return;
  if (surface == pad->current_surface) return;
  ...
  if (!surface || !wlr_surface_accepts_tablet_v2(surface, pad->tablet->tablet_v2)) return;
