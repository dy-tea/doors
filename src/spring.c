...
spring_curve_t *spring_add(const char *name, double stiffness, double damping,
    double mass, double value_epsilon, double velocity_epsilon) {
  ...
  if (existing) {
    ...
    wlr_log(WLR_DEBUG, "spring: updated '%s' (k=%.1f, d=%.1f, m=%.1f)",
  ...
  wlr_log(WLR_DEBUG, "spring: added '%s' (k=%.1f, d=%.1f, m=%.1f)",
    name, stiffness, damping, mass);
  return c;
}
...
double spring_evaluate(spring_curve_t *curve, double dt,
  ...
  if (zeta < 1.0) {
    ...
    v = expTerm * (
      (-c1 * zeta * o + c2 * od) * cosTerm +
      (-c2 * zeta * o - c1 * od) * sinTerm
    );
  } else if (zeta == 1.0) {
    // critically damped
    double c1 = dT;
  ...
  *velocity = v;
  ...
  bool settled = fabs(p - 1.0) < curve->value_epsilon &&
    fabs(v) < curve->velocity_epsilon;
