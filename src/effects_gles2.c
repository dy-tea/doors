...
static void refraction_pass(GLuint src_tex, GLuint dst_fbo, int w, int h,
    struct be_blur_params *p, int refraction_mode) {
  glBindFramebuffer(GL_FRAMEBUFFER, dst_fbo);
  glViewport(0, 0, w, h);
  glActiveTexture(GL_TEXTURE0);
  glBindTexture(GL_TEXTURE_2D, src_tex);
...
static void box_pass(GLuint src_tex, GLuint ping_fbo, GLuint ping_tex,
    GLuint pong_fbo, int w, int h, struct be_blur_params *p) {
...
static bool gles2_init(struct wlr_renderer *r, struct wlr_allocator *a) {
...

  {
    ...
    if (!g->render_fmt) g->render_fmt = fmts ? wlr_drm_format_set_get(fmts, DRM_FORMAT_XRGB8888) : NULL;
    if (!g->render_fmt) {
  ...
  g->u_refraction.refraction_corner_radius_pixels = glGetUniformLocation(g->prog_refraction, "refraction_corner_radius_pixels");
  ...
  g->u_refraction.refraction_texture_repeat_mode = glGetUniformLocation(g->prog_refraction, "refraction_texture_repeat_mode");
...
static bool gles2_output_init(be_output_state_t *state, int width, int height,
...
static void gles2_output_fini(be_output_state_t *state) {
  if (!g) return;
  egl_make_current();
  destroy_fbo((GLuint *)&state->ping.native_handle[0],
      (GLuint *)&state->ping.native_handle[1]);
  destroy_fbo((GLuint *)&state->pong.native_handle[0],
      (GLuint *)&state->pong.native_handle[1]);
...
static void gles2_output_resize(be_output_state_t *state, int width, int height,
    int blur_w, int blur_h) {
  egl_make_current();
  destroy_fbo((GLuint *)&state->ping.native_handle[0],
      (GLuint *)&state->ping.native_handle[1]);
...
static bool gles2_ensure_buffer(struct wlr_buffer **buf, uint64_t native[2],
    int w, int h, struct wlr_renderer *r, struct wlr_allocator *a) {
  if (*buf) return native[0] != 0;
  if (!g->render_fmt) return false;
...
static bool gles2_blit(uint64_t src_tex, uint64_t dst_fbo, int w, int h,
  ...
  if (n_scissor > 0 && scissor) {
    ...
    for (int i = 0; i < n_scissor; i++) {
      glScissor(scissor[i].x1, scissor[i].y1,
          scissor[i].x2 - scissor[i].x1, scissor[i].y2 - scissor[i].y1);
      draw_quad();
    }
    glDisable(GL_SCISSOR_TEST);
  } else {
...
static bool gles2_blur(be_output_state_t *state, uint64_t src_handle,
...
static bool gles2_apply_mica_tint(be_output_state_t *state,
...
static bool gles2_apply_acrylic(be_output_state_t *state,
...
static bool gles2_render_shadow(struct be_shadow_params *p,
    uint64_t dst_fbo) {
  if (!g->prog_shadow) return false;
...
static bool gles2_render_border(struct be_border_params *p,
    uint64_t dst_fbo) {
  if (!g->prog_border) return false;
...
static bool gles2_apply_screen_shader(uint64_t src_tex, uint64_t dst_fbo,
    int w, int h, struct be_screen_shader_params *p) {
  if (!g->screen_shader_prog) return false;
...
static bool gles2_capture_readback(struct wlr_buffer *capture_buffer,
