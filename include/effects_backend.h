...
typedef struct effects_backend_t {
  ...
  bool (*ensure_buffer)(struct wlr_buffer **buf, uint64_t native[2],
  ...
  bool (*blit)(uint64_t src_tex, uint64_t dst_fbo, int w, int h,
      const pixman_box32_t *scissor, int n_scissor);

  bool (*blur)(be_output_state_t *state, uint64_t src_handle,
      int src_w, int src_h, struct be_blur_params *p,
      uint64_t *out_handle);

  bool (*apply_mica_tint)(be_output_state_t *state,
      uint64_t bg_handle, float tint[4], float tint_strength,
      uint64_t dst_fbo, int w, int h);

  bool (*apply_acrylic)(be_output_state_t *state,
  ...
  bool (*capture_readback)(struct wlr_buffer *capture_buffer,
